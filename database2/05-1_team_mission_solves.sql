create table if not exists public.team_mission_solves (
  ...
  difficulty public.leetcode_difficulty null,

  constraint team_mission_solves_pkey
    primary key (id),

  constraint team_mission_solves_team_mission_id_fkey
    foreign key (team_mission_id)
    references public."teamMission" (rid)
    on delete cascade,

  constraint team_mission_solves_team_id_fkey
    foreign key (team_id)
    references public.team (rid)
    on delete cascade,

  constraint team_mission_solves_unique
...
drop policy if exists "team_mission_solves_select_members"
...
drop policy if exists "team_mission_solves_insert_members"
on public.team_mission_solves;
...
drop policy if exists "team_mission_solves_delete_leaders"
on public.team_mission_solves;
