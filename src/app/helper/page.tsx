...
const FEATURE_CARDS = [
  {
    ...
    icon: <i className="fa-solid fa-book-open text-[2rem]"></i>,
  },
  {
    id: 2,
    badge: "AI 분석",
    ...
    buttonText: "코드 분석하기",
    colors: LIGHT_BLUE_THEME,
    icon: <i className="fa-solid fa-code text-[2rem]"></i>,
  },
  {
    id: 3,
    badge: "단계별 힌트",
    ...
    buttonText: "힌트 요청하기",
    colors: LIGHT_BLUE_THEME,
    icon: <i className="fa-regular fa-lightbulb text-[2.3rem]"></i>,
  },
  {
    id: 4,
    badge: "AI 분석",
    ...
    buttonText: "약점 분석하기",
    colors: LIGHT_BLUE_THEME,
    icon: <i className="fa-solid fa-tag text-[2.3rem]"></i>,
  },
];
...
const MOCK_RECOMMENDED_PROBLEMS: RecommendedProblem[] = [
  {
    ...
    platform: "LeetCode",
    title: "Two Sum",
    reason: "BFS를 활용한 최단 거리 탐색 문제입니다.",
    difficulty: "Medium",
  ...
  },
  {
    ...
    platform: "백준",
    title: "연구소",
    reason: "이전에 성공적으로 푼 문제와 유사한 응용 형태입니다. 도전해보세요!",
    difficulty: "Hard",
  ...
  },
  {
    ...
    platform: "백준",
    title: "바이러스",
    reason: "DFS 개념을 완벽히 복습하기에 가장 적합한 문제로 분석되었습니다.",
    difficulty: "Medium",
    difficultyColor: "bg-yellow-100 text-yellow-700",
    tags: ["그래프", "DFS"],
    estimatedTime: "25분",
  ...
  },
  {
    ...
    platform: "백준",
    title: "어린 왕자",
    reason: "수학적 사고력을 기르기 위해 AI가 특별히 선택한 추천 문제입니다.",
    difficulty: "Easy",
...
export default function AssistantLanding() {
  ...
  return (
    <div
      className={`relative flex flex-col items-center justify-center min-h-screen px-4 py-12 mt-5 ${notoSansKr.className}`}
    >
        ...
        {FEATURE_CARDS.map((card) => (
          ...
          >
            <div className="flex gap-5 mb-8">
              <div
                className={`w-24 h-24 rounded-full flex items-center justify-center shrink-0 ${card.colors.iconBg} ${card.colors.iconText}`}
              >
                {card.icon}
              </div>
              <div className="flex flex-col pt-1">
                <div
                  className={`w-fit px-3 py-1 rounded-full text-[13px] font-bold mb-2 flex items-center ${card.colors.badgeBg} ${card.colors.badgeText}`}
                >
                  {card.badge}
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {card.tags.map((tag, idx) => (
                    <span
                      key={idx}
                      className={`px-3 py-1 text-[12px] font-medium rounded-full ${card.colors.tagBg} ${card.colors.tagText}`}
                    >
                      {tag}
                    </span>
                  ))}
            ...
            <div className="p-8 bg-gray-50 overflow-y-auto">
              {isRecommending ? (
                <div className="flex flex-col items-center justify-center py-32">
                  <svg
                    className="animate-spin h-12 w-12 text-blue-500 mb-6"
                  ...
                  {recommendedProblems.map((prob) => (
                    ...
                    >
                      <div className="flex justify-between items-start mb-3">
                        <div>
                          <div className="text-sm font-bold text-blue-500 mb-0.5 tracking-tight">
                            #{prob.id}
                          </div>
                        ...
                        <span
                          className={`px-2.5 py-1 rounded-full text-xs font-bold shrink-0 ${prob.difficultyColor}`}
                        >
                          {prob.difficulty}
                        </span>
                      </div>
                        ...
                        <div className="flex justify-between items-center text-sm">
                          <div className="flex items-center gap-1.5 text-gray-600">
                            ...
                            <span className="font-medium">
                              {prob.solvedCount.toLocaleString()}명 도전
                            </span>
                          </div>
                      ...
                      <div className="flex flex-wrap gap-1.5 mb-5">
                        {prob.tags.map((tag, idx) => (
                          <span
                            key={idx}
                            className="px-2.5 py-1 bg-blue-50 text-blue-600 text-xs font-bold rounded-lg"
                          >
                            # {tag}
                          </span>
                        ))}
