        ...
        <div className="flex flex-col gap-4">
          {data.map((prob: any) => {
            const progressRatio = Math.round(
              (prob.solvedCount / prob.totalCount) * 100,
            );

            return (
              <div
                key={prob.id}
                className="border border-gray-200 rounded-xl p-4 transition-colors hover:border-indigo-200"
              >
                {/* 문제 제목 및 진행률 */}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                  {prob.members.map((member: any) => (
                    ...
                    >
                      <div
                        className={`shrink-0 w-7 h-7 rounded-full flex items-center justify-center text-white text-[11px] font-bold shadow-sm
                      ...
                      <div className="flex flex-col overflow-hidden">
                        ...
                        <div className="flex items-center gap-1 mt-0.5">
                          {member.status === "completed" && (
                            <i className="fa-solid fa-check text-[9px] text-green-600" />
                          )}
                          {member.status === "working" && (
                            <i className="fa-solid fa-pen text-[9px] text-orange-500" />
                          )}
                          {member.status === "pending" && (
                            <i className="fa-solid fa-minus text-[9px] text-gray-400" />
                          )}
                          <span
...
function StudySessionStats({ data }: { data: any }) {
  ...
  const progressPercent = hasActiveStudy
      ...
      {!hasActiveStudy ? (
        <div className="flex flex-col items-center justify-center py-12 text-center border border-dashed border-gray-200 rounded-xl bg-gray-50/50 mb-2">
          ...
          <p className="text-gray-500 text-[13px] mt-1.5">
            새로운 스터디를 시작하고 멤버들과 진행률을 공유해보세요!
          </p>
        </div>
      ) : (
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-10 w-full">
          ...
          <div className="flex flex-col flex-1 w-full mt-2">
            <h3 className="text-[15px] font-bold text-[#1e293b] mb-5">
            ...
            <div className="flex flex-col gap-4 mb-6 w-full">
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex flex-col">
                  <span className="text-[13px] font-bold text-[#1e293b]">
                    다음 회차 ({data.presentSession + 1}회차) 준비 중
                  </span>
                  <span className="text-[12px] font-medium text-gray-500 mt-0.5">
                    예정 난이도 : Medium
                  </span>
...
export default function AlgorithmDashboard() {
  ...
  useEffect(() => {
    const getTeamInfo = async () => {
      ...
      members.push({
        ...
        suffix: "",
        email: data.teamLeader + "@gmail.com",
        initial: data.teamLeader[0].toUpperCase(),
        role: "팀장",
        isMe: true,
      });
      data.UserList.forEach((elem: any) => {
        members.push({
          ...
          suffix: "",
          email: elem.boj_handle,
          initial: elem.boj_handle[0].toUpperCase(),
          role: "",
          isMe: false,
        });
      });
  ...
  useEffect(() => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const krDate = new Date(
        date.toLocaleDateString("en-US", { timeZone: "Asia/Seoul" }),
      );

      let dateFormat = krDate.toLocaleDateString("sv-SE");

      dateData.push({
        day: dateFormat.slice(8, 10),
        label: i == 0 ? "(오늘)" : days[date.getDay()],
      });
    }
    setDates(dateData);
  }, []);
  ...
  useEffect(() => {
    const getActivity = async () => {
      ...
      problems.forEach((elem: any) => {
        console.log(elem);
        const dateSeries = new Date(
          new Date(elem.solved_at).toLocaleDateString("en-US", {
            timeZone: "Asia/Seoul",
          }),
        );
        const today = new Date();
  ...
  // let user = "chcl77"
  const isTeamMember =
    teamInfo?.teamLeader === user ||
    teamInfo?.UserList?.some((member: any) => member.boj_handle === user);
  ...
  return (
    // bg-[#F9FAFB] 제거
    <div
      className={`min-h-screen p-8 text-gray-800 relative ${notoSansKr.className}`}
    >
      <link
        rel="stylesheet"
      ...
      <div className="max-w-[1200px] mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <section className="bg-white rounded-xl border border-blue-100 p-5 shadow-sm">
              <div className="flex justify-between items-center mb-4 border-b border-slate-200 pb-[15px]">
                <div className="flex items-center gap-2 text-blue-600">
                  ...
                  <span className="text-blue-600 text-[15px] font-semibold">
                    · {missionList.length}개
                  </span>
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    <i className="fa-solid fa-arrows-rotate"></i> 스터디 생성 /
                    초기화
                  </button>
                </div>
              </div>
              ...
              {isLoadingMissions ? (
                ...
                <div className="flex flex-col items-center justify-center py-14 text-center border border-dashed border-blue-200 rounded-xl bg-blue-50/10">
                  ...
                  <p className="text-gray-400 text-xs mt-1">
                    상단의 '새로운 스터디 생성' 버튼을 눌러 난이도와 태그를
                    설정해보세요.
                  </p>
                </div>
              ) : (
                        ...
                        <div className="border border-gray-200 rounded-lg p-4 flex flex-col flex-1 bg-white hover:shadow-md transition-shadow">
                          <div className="flex justify-between items-center mb-3">
                            ...
                            <div className="flex items-center gap-1 text-xs text-gray-500">
                              <span
                                className={`${isCompleted ? "text-green-500" : ""} flex items-center gap-1 mr-[5px]`}
                              >
                                {/* <i className="fa-solid fa-arrows-rotate"></i> 20:00 */}
                              </span>
                              <a href={mission.link} target="_blank">
                                <button
                                  className="bg-none border-none cursor-pointer text-gray-400 text-xs p-0.5 shrink-0 transition-colors duration-150 hover:text-[#4F6EF7]"
                                  title="문제 보기"
                                >
                          ...
                          <div className="flex items-start gap-2 mb-4">
                            <div
                              className={`w-5 h-5 mt-0.5 rounded-sm flex items-center justify-center text-[10px] font-bold text-white bg-[#0087ff] `}
                            >
                              <i className="fa-solid fa-code"></i>
                            </div>
                            <h3 className="font-bold text-[15px] flex-1 line-clamp-2 break-keep leading-tight">
                              {mission.title}
                            </h3>
                          </div>
                        ...
                        {isTeamMember && (
                          <button
                            onClick={() =>
                              !isCompleted &&
                              !isLoading &&
                              handleVerifyClick(mission.id, mission.titleSlug)
                            }
            ...
            <section className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="font-semibold">팀 활동</span>
              </div>
              ...
              <div className="flex justify-between items-end mb-2">
                <div className="flex gap-2 text-xs text-gray-500">
                  ...
                  <button className="text-blue-600 font-bold bg-blue-50 px-2 py-1 rounded">
                    7일
                  </button>
                    ...
                    {activity.map((record: any, i: any) => (
                      <tr key={i}>
                        <td className="py-1 pr-4 text-left">
                          <div
                            className={`flex items-center gap-2 p-1.5 rounded-md ${record.isMe ? "bg-blue-50/50" : ""}`}
                          >
                            <div
                              className={`w-6 h-6 rounded-full flex items-center justify-center text-white text-xs ${record.isMe ? "bg-blue-500" : "bg-gray-400"}`}
                            >
                              {record.initial}
                            </div>
                        ...
                        {record.records.map((val: any, j: any) => (
                          <td key={j} className="p-1">
                            <div
                              className={`w-full h-8 flex items-center justify-center rounded-sm text-xs font-medium ${getCellColor(val)}`}
                            >
                              {val}
                            </div>
                          </td>
          ...
          <div className="space-y-6">
            {/* 팀 정보 부분 */}
            <section className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
              <h2 className="font-semibold mb-4">팀 정보</h2>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex items-center gap-2">
                    {missionInfo?.[0]?.difficulty == "Easy" ? (
                      <span className="bg-[#ecfdf5] text-[#059669] px-2 py-0.5 rounded text-xs font-semibold">
                        Easy
                      </span>
                ...
                <div className="pt-2">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {infoTags.map((tag) => (
                      <span
                        key={tag}
                        className="bg-blue-50 text-blue-600 text-xs px-2 py-1 rounded-md font-medium border border-blue-100"
                      >
                        {tag}
                      </span>
                    ))}
            ...
            <section className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
              ...
              <div className="space-y-1">
                {teamMembers.map((member: any, i: any) => (
                  <div
                    key={i}
                    className={`flex items-center justify-between p-2 rounded-lg ${member.isMe ? "bg-blue-50/50" : "hover:bg-gray-50"}`}
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-sm font-medium ${member.isMe ? "bg-blue-600" : "bg-gray-400"}`}
                      >
                        {member.initial}
                      </div>
                      <div className="flex flex-col">
                    ...
                    return (
                      ...
                      >
                        <span className="text-sm font-bold flex items-center justify-center gap-1">
                          {item.label}
                          {isSelected && (
                            <i className="fa-regular fa-circle-check text-xs text-blue-500 ml-0.5"></i>
                          )}
                        </span>
                        <span className="text-[10px] text-gray-400 mt-1 block break-all whitespace-pre-line leading-tight">
                          {item.desc}
                        </span>
                      </button>
                    );
