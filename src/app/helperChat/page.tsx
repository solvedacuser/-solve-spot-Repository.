...
const MessageFormatter = ({
  ...
  return (
    <div
      className={`flex flex-col gap-2.5 w-full text-[14.5px] ${role === "user" ? "items-end" : "items-start"}`}
    >
      <link
        rel="stylesheet"
      ...
      {parts.map((part, index) => {
        ...
        if (part.startsWith("```")) {
          ...
          return (
            <div
              key={index}
              className="w-[85%] max-w-full rounded-2xl overflow-hidden border border-gray-200 bg-gray-50 my-1 shadow-sm"
            >
...
export default function DesktopSplitCodeChatUI() {
  ...
  const recommendedProblems: RecommendedProblem[] = [
    ...
    {
      id: "p2",
      title: "정렬된 배열에서 이진 탐색 구현",
      difficulty: "Medium",
      category: "Algorithm",
    },
    {
      id: "p3",
      title: "React 커스텀 훅(useFetch) 작성",
      difficulty: "Medium",
      category: "React",
    },
  ];
    ...
    try {
      const response = await fetch("/api/v1/chat", {
        ...
        body: JSON.stringify({
          chatLog:
            queryType == "weakness"
              ? [{ id: "0-1", role: "user", content: weaknessQuery }]
              : messages,
        }),
      });
  ...
  return (
    <div
      className={`flex h-screen w-full font-sans overflow-hidden items-center justify-center p-4 md:p-8 ${notoSansKr.className}`}
    >
      <div className="flex flex-row gap-6 w-full max-w-[1400px] h-full max-h-[85vh]">
        <div className="hidden lg:flex flex-col w-80 bg-white rounded-[2rem] shadow-xl border border-gray-200 overflow-hidden flex-shrink-0">
                  ...
                  {aiLogs.map((log) => (
                    ...
                    >
                      <div className="relative z-10 flex-shrink-0 w-6 h-6 rounded-full border-[3px] border-white bg-white flex items-center justify-center shadow-sm">
                        <div
                          className={`w-2 h-2 rounded-full ${log.status === "success" ? "bg-blue-500" : "bg-amber-400 animate-pulse"}`}
                        ></div>
                      </div>
          ...
          <div className="px-6 py-6 border-t border-gray-100 bg-white">
            <div className="flex items-center gap-2 mb-4 px-1">
              ...
              <h3 className="text-[10px] font-bold text-gray-300 uppercase tracking-[0.2em] whitespace-nowrap">
                Quick Start
              </h3>
              <div className="h-[1px] flex-1 bg-gray-100"></div>
            </div>
              ...
              {[
                {
                  ...
                  prompt: "현재 내 코드에 대한 리뷰를 구성해줘",
                  color: "blue",
                },
                {
                  label: "Performance",
                  icon: "fa-gauge-high",
                  prompt: "현재 내 코드의 성능 최적화 방안을 알려줘",
                  color: "purple",
                },
                {
                  label: "Bug Detection",
                  icon: "fa-bug",
                  prompt: "현재 내 코드에서 발생할 수 있는 버그를 찾아줘",
                  color: "red",
                },
              ].map((item) => (
                <button
                ...
                >
                  <div
                    className={`w-8 h-8 rounded-xl bg-gray-50 flex items-center justify-center text-xs text-gray-500 group-hover:bg-${item.color}-50 group-hover:text-${item.color}-600 transition-colors`}
                  >
        ...
        <div className="flex-1 bg-white rounded-[2rem] shadow-2xl border border-gray-200 flex flex-col overflow-hidden min-w-0">
          <header className="px-8 py-5 border-b border-gray-100 flex items-center gap-3 bg-white">
            <div className="w-10 h-10 rounded-full bg-purple-100 flex items-center justify-center text-lg shadow-sm">
          ...
          <div
            id="chat-container"
            className="flex-1 overflow-y-auto p-6 bg-gray-50/30 relative scroll-smooth"
          >
            <div className="space-y-8">
              ...
              {isLoading && (
                <div className="flex items-start gap-3.5 flex-row">
                  <div className="w-9 h-9 rounded-full bg-purple-100 flex items-center justify-center flex-shrink-0 shadow-sm">
                    <i className="fa-solid fa-graduation-cap"></i>
                  </div>
          ...
          <div className="p-6 bg-white border-t border-gray-100">
            <form
              ...
              className="relative flex items-end shadow-sm border border-gray-200 rounded-2xl bg-gray-50 focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-transparent transition-all focus-within:bg-white"
            >
              <textarea
                value={input}
